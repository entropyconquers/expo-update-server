...
app.post("/api/register-app", async (c) => {
  try {
...
app.put("/api/apps/:slug/certificate", async (c) => {
  try {
...
app.get("/api/certificate/:slug", async (c) => {
  try {
    const slug = c.req.param("slug");
...
app.put("/api/apps/:slug/settings", async (c) => {
  try {
    const slug = c.req.param("slug");
    const body = await c.req.json();
...
app.put("/api/release/:id", async (c) => {
  try {
...
app.put("/api/apps/:slug/release/:uploadId", async (c) => {
  try {
    const slug = c.req.param("slug");
...
app.get("/manifest", async (c) => {
  try {
    ...
    // Generate base URL from request host
    const baseUrl = `https://${c.req.header("host")}`;
...
app.get("/api/apps", async (c) => {
  try {
    const { results } = await c.env.DB.prepare(
      `
      SELECT 
        id, private_key, certificate, created_at, 
        updated_at, name, description, owner_email, auto_cleanup_enabled
      FROM apps 
      ORDER BY created_at DESC
    `
    ).all();
    ...
    const appsData = {
      ...
      apps: results.map((app: any) => ({
        ...
        description: app.description,
        ownerEmail: app.owner_email,
        certificateStatus: app.certificate ? "configured" : "not_configured",
        hasCodeSigning: !!app.certificate,
        updateUrl: `${baseUrl}/manifest?project=${app.id}`,
        settings: {
          autoCleanupEnabled: !!app.auto_cleanup_enabled,
        },
      })),
    };
...
// Static asset serving - Frontend dashboard
// This serves the Next.js static files for the dashboard
app.get("/*", async (c) => {
  try {
    // Skip API routes - let them be handled by specific handlers above
    ...
    if (
      ...
      // If we reach here, it means the route wasn't found
    ...
    // For SPA routing, return index.html for any non-asset path
    // that doesn't match static files (like .js, .css, .png, etc.)
    if (!path.includes(".") || path.endsWith(".html")) {
      const indexResponse = await c.env.ASSETS.fetch(
        new Request(c.req.url.replace(path, "/index.html"))
      );
    ...
    // If no static asset or index.html found, return 404
    return c.text("Not found", 404);
  } catch (error) {
    console.error("Static asset serving failed:", error);
