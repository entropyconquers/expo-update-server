...
import AppDetails from "./app-details-view";

// Individual App View Component
function AppDetailsView({ slug }: { slug: string }) {
  return <AppDetails slug={slug} />;
}

// Apps List View Component
          ...
          <div className="space-y-4">
            {filteredApps.map((app) => (
              <Card
                ...
                onClick={() => (window.location.href = `/apps?id=${app.slug}`)}
                role="button"
                tabIndex={0}
                onKeyDown={(e) => {
                  if (e.key === "Enter" || e.key === " ") {
                    e.preventDefault();
                    window.location.href = `/apps?id=${app.slug}`;
                  }
                }}
              >
                <CardContent className="p-6">
                      ...
                      <div className="flex items-start justify-between">
                        ...
                        >
                          ...
                          {app.certificateStatus === "not_configured" && (
                            ...
                            >
                              <Link
                                href={`/apps?id=${app.slug}&tab=certificates`}
                              >
                          ...
                          <DropdownMenu>
                            ...
                            <DropdownMenuContent align="end" className="w-48">
                              ...
                              >
                                <Link
                                  href={`/apps?id=${app.slug}&tab=settings`}
                                >
...
// Client component that uses search params (wrapped in Suspense)
function AppsPageClient() {
  const searchParams = useSearchParams();
  const appId = searchParams.get("id");

  // If there's an app ID in the query params, show the individual app view
  if (appId) {
    return <AppDetailsView slug={appId} />;
  }
...
// Main Apps Page Component with Suspense wrapper
